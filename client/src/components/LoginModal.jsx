...
const LoginModal = ({ isOpen, onClose, onSwitchToRegister }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success && result.otpSent) {
      ...
      toast.dismiss();
      showErrorToast('Could not start sign-in. Please try again.');
    }
  ...
  const handleCancelOTP = () => {
    setShowOTPModal(false);
    setOtpEmail('');
    toast('Enter your password again to request a new code.', { icon: 'ℹ️' });
  };
      ...
      <Modal isOpen={isOpen} onClose={onClose} title="Sign In">
        <div className="space-y-6">
          ...
          <div className="text-center">
            <p className="text-gray-600">Welcome back! Sign in with email — we will send a one-time code to verify it is you.</p>
          </div>
